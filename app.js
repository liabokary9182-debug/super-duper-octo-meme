(function(){
  ...
  function buildFinalAnswer(){
    ...
    switch(focus.label){
      ...
      case "关系中的疑问":
        answer=/会不会|喜欢|在乎|找我|回来|联系|心意/.test(state.question)?"现在不能从牌面确定对方会怎么想或怎么做；请看对方实际的行动，并在合适的时候直接沟通。":(cautious?"这段关系先不要急着推进；弄清彼此的期待和边界后再决定。":"这段关系可以继续了解；把你的需要说清楚，再看双方能否回应。");break;
      case "工作中的方向":
        answer=cautious?"工作上的决定先缓一缓；确认现实条件和阻力后再行动。":"可以推进你问的工作方向；先做一次具体尝试，再根据反馈调整。";break;
      case "眼前的选择":
        answer=cautious?"我倾向于先不要定案；核对代价和关键条件后再选。":"可以先朝更符合你核心需要的选项行动，并用小步尝试验证。";break;
      case "身心状态":
        answer="先照顾眼下最消耗你的部分，并寻求可信赖的支持；持续影响生活时请联系专业人士。";break;
      case "自己的成长":
        answer=cautious?"先调整让你卡住的方法，再迈出下一步。":"可以开始你想要的改变，先选一个能持续的小行动。";break;
      default:
        answer=/会不会|能否|有没有可能/.test(state.question)?"现在不能仅凭牌面断定这件事会不会发生；先确认你能核实的事实，再决定可做的一步。":/该不该|要不要|是否|能不能|可不可以|适不适合|值得吗/.test(state.question)?(cautious?"我倾向于先不要仓促推进；确认关键条件后再决定。":"可以先尝试，但请用现实反馈确认这条路是否适合你。"):("先从与你所问的事直接相关、可以核实或改变的一步入手。 "+focus.action);
    }
    if(state.concern&&/收入|工资|经济|预算|房贷|存款/.test(state.question))answer+=" 特别要先算清收入变化与必要开支。";
    return "给你的回答："+answer+" 这样判断，是因为最后的"+last.card.name+"（"+(last.reversed?"逆位":"正位")+"）提醒你「"+cardHint+"」"+(reverseCount>=2?"，而多张逆位也显示阻力尚在":"")+"。";
  }
